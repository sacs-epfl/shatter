...
class Attacker:
    ...
    def grad_inv(self, grad, x, onehot, model, logger=None):
        ...
        latent_data = np.random.rand(
            x.shape[0], 3, int(sample_size[0] / self.sf), int(sample_size[1] / self.sf)
        )
        ...
        scheduler = optim.lr_scheduler.CosineAnnealingLR(
            dummy_data_optimizer, T_max=self.T_max, eta_min=0.1 * self.eta
        )
        ...
        for iters in range(self.T_max):
            ...
            def closure():
                ...
                pred = model(
                    F.interpolate(dummy_data, scale_factor=self.sf, mode="bicubic")
                )
            ...
            if iters % self.printevery == 0:
                ...
                logger.info(
                    "iter: {:d} loss: {:.4e}".format(iters, current_loss.item())
                )
            ...
            def closure():
                ...
                for i in range(tau):
                    ...
                    g = torch.autograd.grad(
                        dummy_loss,
                        meta_net.parameters.values(),
                        retain_graph=True,
                        create_graph=True,
                        only_inputs=True,
                    )

                    meta_net.parameters = OrderedDict(
                        (name, param - self.fed_lr * grad_part)
                        for ((name, param), grad_part) in zip(
                            meta_net.parameters.items(), g
                        )
                    )

                meta_net.parameters = OrderedDict(
                    (name, (param_origin - param) / self.fed_lr)
                    for ((name, param), (name_origin, param_origin)) in zip(
                        meta_net.parameters.items(), net0.items()
                    )
                )
...
def grad_inv(attacker, grad, x, onehot, model, config, logger):
    if config.fedalg != "fedavg":
        ...
        dummy_data = attacker.multistep_attack(
            grad, x, onehot, model, config.tau, logger
        )
