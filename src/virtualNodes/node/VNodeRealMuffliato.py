...
class VNodeReal(VNode):
    ...
    def run(self):
        ...
        noise_generator.manual_seed(self.dataset.random_seed * 100 + self.uid)
        ...
        for iteration in range(self.iterations):
            ...
            if iteration == 0 or iteration % self.train_after == 0:

                # Learning rate decay
                if (
                    iteration > 0
                    and iteration % (self.reduce_lr_after * self.train_after) == 0
                ):
                ...
                if self.noise_std > 0:
                    ...
                    for key in state_dict:
                        state_dict[key] += (
                            torch.randn(
                                state_dict[key].shape, generator=noise_generator
                            )
                            * self.noise_std
                        )
                    self.model.load_state_dict(state_dict)
            ...
            for i, to_send in enumerate(to_send_list):
                ...
                to_send["not_trained"] = not_trained
            ...
            if self.dataset.__training__ and (
                iteration == 0
                or ((iteration + 1) % (self.train_after * self.train_evaluate_after))
                == 0
            ):
            ...
            if self.dataset.__testing__ and (
                iteration == 0
    ...
    def instantiate(
        ...
        *args
        ...
        self.noise_std = nodeConfigs["noise_std"]
        self.train_after = nodeConfigs["train_after"]
